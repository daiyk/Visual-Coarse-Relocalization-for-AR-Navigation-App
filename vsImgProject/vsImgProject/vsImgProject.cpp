...
int main(int argc, const char* argv[]) {
    ...
    try
    {
        readResult = readFiles(argc, argv, trainPaths, testPaths);
    }
    ...
    if (readResult.count("inputs")) {
        matchingtest(trainPaths);
        return 0;
    }
    VL_PRINT("!------- Start feature detection with OpenCV and VLFeat ------!\n");
    if (readResult.count("both")|| readResult.count("vlfeat")) {
        VL_PRINT("!------- VLFeat ------!\n");
        clock_t sTime = clock();
        ...
        if (readResult.count("path")) {
            //train k-means classifier
            //free memory since keypoints during training is not useful 
            std::vector<KeyPoint>().swap(keypoints);
            vl_visual_word_compute(allDescrips, kCenters);
            std::vector<KeyPoint> kpts;
            write_to_file("vlfeat", kpts, kCenters);
        }
        else if(readResult.count("img")){ //display the detection result for single image
            Mat outImg, purImg;
        ...
        cout << "-> vlfeat learning totally spent " << (clock() - sTime) / double(CLOCKS_PER_SEC) << " sec......" << endl;
    }
    
    //openCV pipeline
    if (readResult.count("both") || readResult.count("opencv")) {
        VL_PRINT("!------- OpenCV ------!\n");
        clock_t sTime = clock();
        try {
            openCVimg_descips_compute(trainPaths, allDescrips, keypoints);
        }
        catch (std::invalid_argument& e) {
        ...
        if (readResult.count("path")) {
            std::vector<KeyPoint>().swap(keypoints);
            openCV_visual_words_compute(allDescrips, kCenters);
            std::vector<KeyPoint> kpts;
            write_to_file("opencv", kpts, kCenters);
        }
        else if(readResult.count("img")){
            Mat outImg, purImg;
            outImg = imread(trainPaths[0]);
            purImg = outImg;
        ...
        cout << "-> opencv pipeline totally spent " << (clock() - sTime) / double(CLOCKS_PER_SEC) << " sec......" << endl;
        //write important data to file
    }
    else
        cout << "ERROR: tool option should be either \"opencv\" or \"vlfeat\"."<<endl;
