...
int kernelTable(string &kernel_name)
{
  if (kernel_name.compare("E") == 0)
    return 1;
  else if (kernel_name.compare("V") == 0)
    return 2;
  else if (kernel_name.compare("VE") == 0)
    return 3;
  else if (kernel_name.compare("H") == 0)
    return 4;
  else if (kernel_name.compare("EG") == 0)
    return 5;
  else if (kernel_name.compare("VG") == 0)
    return 6;
  else if (kernel_name.compare("VEG") == 0)
    return 7;
  else if (kernel_name.compare("GR") == 0)
    return 8;
  else if (kernel_name.compare("ER") == 0)
    return 9;
  else if (kernel_name.compare("kR") == 0)
    return 10;
  else if (kernel_name.compare("WL") == 0)
    return 11;
  else
    return 1;
}
...
const char *getVlabel(igraph_t g, igraph_vector_t vtypes, igraph_strvector_t vnames)
{
  ...
  else if (igraph_strvector_size(&vnames) == 2)
  {
    for (int i = 0; i < igraph_strvector_size(&vnames); i++)
    {
      ...
      {                          //as long as it doesn't equals to "id" then record it
...
void WLKernelMatrix(vector<MatrixXi> &E, vector<vector<int>> &V_label, vector<int> &num_v, vector<int> &num_e, vector<int> &degree_max, int h_max, MatrixXd &K_mat)
{
  ...
  { //n: num of graphs
    ...
    {                                           // num_v: num of vertex for each graph
  ...
  for (int i = 0; i < v_all; i++)
  {
    ...
    { //when there is a jump in label values (or final value), which means the current stage all same value's label has been accumulated.
...
void productAdjacency(MatrixXi &e1, MatrixXi &e2, vector<int> &v1_label, vector<int> &v2_label, MatrixXi &H, SparseMatrix<double> &Ax)
{
  ...
  for (int i = 0; i < e1.rows(); i++)
  {
    ...
    {
      if (v1_label[e1(i, 0)] == v2_label[e2(j, 0)] && v1_label[e1(i, 1)] == v2_label[e2(j, 1)] && e1(i, 2) == e2(j, 2))
      {
        v.push_back(T(H(e1(i, 0), e2(j, 0)), H(e1(i, 1), e2(j, 1)), 1.0));
        v.push_back(T(H(e1(i, 1), e2(j, 1)), H(e1(i, 0), e2(j, 0)), 1.0));
      }
      if (v1_label[e1(i, 0)] == v2_label[e2(j, 1)] && v1_label[e1(i, 1)] == v2_label[e2(j, 0)] && e1(i, 2) == e2(j, 2))
      {
